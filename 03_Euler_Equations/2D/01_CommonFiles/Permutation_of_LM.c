...
void Permutation_of_LM(ParametersType *Parameters, MatrixDataType *MatrixData, FemStructsType *FemStructs)
{
					...
					for (J=0; J<3; J++){
						...
						for (JJ=0; JJ<NDOF; JJ++){	
							if (Node[vb].Type[JJ] != 0){
								insertNode(List, Node[va].invP_id[II] + neq_bef, Node[vb].invP_id[JJ] + neq_bef, &nnz);
							}
						}
					}
				}
