...
C 4/10/95. IQ in MA50B made assumed size
C 1/11/95. IQ in MA50C made assumed size.
C 1/11/95. STRSV not called for zero-sized array.
C 14/2/96. NP initialized to 0.
C 13/11/97 INFO(4) and INFO(6) in MA50A made to reflect the situation
C          at the point of failure in the case of insufficient storage.
C 17/3/98  In MA50A, copy a row forward if there is space at its front,
...
C          increased and ICNTL(8) added.
      ...
      SUBROUTINE MA50A(M,N,NE,LA,A,IRN,JCN,IQ,CNTL,ICNTL,IP,NP,JFIRST,
...
C    floating-point operations needed for the factorization.
      ...
      REAL ZERO,ONE
      PARAMETER (ZERO=0E0,ONE=1.0)

      REAL ALEN,AMULT,ANEW,ASW,AU,COST,CPIV
...
C Initial printing
     ...
     +     ' Entering MA50A with M =',M,' N =',N,' NE =',NE,' LA =',LA,
...
C No pivot found. Switch to full matrix processing.
     ...
     +       ' Warning message from MA50A: no suitable diagonal pivot',
...
C Column must be moved to the beginning of available storage.
  510       IF (DISPC+LENC(J)+IFILL.GT.LA+1) THEN
               INFO(2) = INFO(2) + 1
               CALL MA50D(LA,A,IRN,IQ,N,DISPC,.TRUE.)
               JBEG = IQ(J)
               JEND = JBEG + LENC(J) - 1
               PIVBEG = IQ(JPIV)
               PIVEND = PIVBEG + LENC(JPIV) - 1
...
C Compress.
                     INFO(2) = INFO(2) + 1
                     CALL MA50D(LA,A,JCN,IP,M,DISPR,.FALSE.)
...
C Test for rank deficiency
      ...
      IF (MP.GT.0 .AND. ICNTL(3).GT.2) THEN
         WRITE (MP,'(A,I6,A,F12.1/A,7I8)') ' Leaving MA50A with NP =',
...
C Error conditions.
  690 INFO(1) = -1
     ...
     +    ' **** Error return from MA50A ****',' M =',M,' N =',N
      GO TO 750
  700 INFO(1) = -2
      IF (LP.GT.0) WRITE (LP,'(/A/(A,I10))')
     +    ' **** Error return from MA50A ****',' NE =',NE
      GO TO 750
  705 INFO(4) =  NEFACT + NERED
      INFO(6) = INFO(6) + NDROP
  710 INFO(1) = -3
      IF (LP.GT.0) WRITE (LP,'(/A/A,I9,A,I9)')
     +    ' **** Error return from MA50A ****',
     +    ' LA  must be increased from',LA,' to at least',INFO(3)
      GO TO 750
  720 INFO(1) = -4
      IF (LP.GT.0) WRITE (LP,'(/A/(3(A,I9)))')
     +    ' **** Error return from MA50A ****',' Entry in row',I,
     +    ' and column',J,' duplicated'
      GO TO 750
  730 INFO(1) = -5
      IF (LP.GT.0) WRITE (LP,'(/A/(3(A,I9)))')
     +    ' **** Error return from MA50A ****',' Fault in component ',
     +    PIVOT,' of column permutation given in IFIRST'
      GO TO 750
  740 INFO(1) = -6
      IF (LP.GT.0) WRITE (LP,'(/A/(3(A,I9)))')
     +    ' **** Error return from MA50A ****',' ICNTL(4) = ',ICNTL(4),
     +    ' when ICNTL(6) = 2'
  750 END


      SUBROUTINE MA50B(M,N,NE,JOB,AA,IRNA,IPTRA,CNTL,ICNTL,IP,IQ,NP,
...
C      holds the vector IPIV output by MA50G.
...
C Check input values
     ...
     +       ' **** Error return from MA50B ****',' M =',M,' N =',N
         GO TO 550
      END IF
      IF (NE.LE.0) THEN
         INFO(1) = -2
         IF (LP.GT.0) WRITE (LP,'(/A/A,I6)')
     +       ' **** Error return from MA50B ****',' NE =',NE
         GO TO 550
      END IF
      IF (NP.LT.0 .OR. NP.GT.N) THEN
         INFO(1) = -7
         IF (LP.GT.0) WRITE (LP,'(/A/A,I8,A,I8)')
     +       ' **** Error return from MA50B ****',' NP =',NP,' N =',N
         GO TO 550
      END IF
     ...
     +          ' **** Error return from MA50B ***',' Call with JOB=',
     ...
     +       ' **** Error return from MA50B ****',' JOB =',JOB
         GO TO 550
      END IF
...
C Print input data
     ...
     +       ' Entering MA50B with M =',M,' N =',N,' NE =',NE,' JOB =',
...
C Factorize full block
         IF (ICNTL(5).GT.1) CALL MA50G(MF,NF,FACT(IF1),MF,ICNTL(5),
     +                                 CNTL(4),IRNF(IF1+MF),RANK)
         IF (ICNTL(5).EQ.1) CALL MA50F(MF,NF,FACT(IF1),MF,CNTL(4),
     +                                 IRNF(IF1+MF),RANK)
         IF (ICNTL(5).LE.0) CALL MA50E(MF,NF,FACT(IF1),MF,CNTL(4),
     +                                 IRNF(IF1+MF),RANK)
         INFO(5) = INFO(5) + RANK
  ...
  450    CONTINUE
     ...
     +     ' Leaving MA50B with IRNF(2) =',IRNF(2),
     +     ' RINFO(1) =',RINFO(1),
...
C LFACT is much too small or ICNTL(8)/=0. Patch up IP and quit.
  ...
  520 INFO(1) = -3
      IF (LP.GT.0) THEN
         WRITE (LP,'(/A)')' **** Error return from MA50B **** '
  ...
  530 INFO(1) = - (7+K)
     ...
     +    ' **** Error return from MA50B **** ',
     +    ' Small pivot found in column',K,' of the permuted matrix.'
      GO TO 550
  540 INFO(1) = -4
      IF (LP.GT.0) WRITE (LP,'(/A/(3(A,I9)))')
     +    ' **** Error return from MA50B ****',' Entry in row',I,
     +    ' and column',J,' duplicated'
  550 END

      SUBROUTINE MA50C(M,N,ICNTL,IQ,NP,TRANS,LFACT,FACT,IRNF,IPTRL,
     +                 IPTRU,B,X,W,INFO)
...
C Make some simple checks
     ...
     +    '(/2(A,I6),A,I4,A,L2)') ' Entering MA50C with M=',M,' N =',N,
     +    ' NP =',NP,' TRANS =',TRANS
      IF1 = IPTRL(N) + 1
      MF = IRNF(2)
      NF = N - NP
...
C Backsubstitute through the full part of (PL) trans.
   ...
   80    CONTINUE
         IF (MF.GT.0 .AND. NF.GT.0) THEN
            CALL MA50H(TRANS,MF,NF,FACT(IF1),MF,IRNF(IF1+MF),X,ICNTL(5))
         ELSE
            DO 90 I = 1,MF
               X(I) = ZERO
   90       CONTINUE
...
C Backsubstitute through the packed part of (PL) trans.
  ...
  120    CONTINUE
         IF (MP.GT.0 .AND. ICNTL(3).GT.3) WRITE (MP,
     +       '(A/(4X,5F10.4))') ' Leaving MA50C with X =', (X(I),I=1,M)
C
      ELSE
         IF (MP.GT.0 .AND. ICNTL(3).GT.3) WRITE (MP,
...
C Forward substitution through the full part of PL
         IF (MF.GT.0 .AND. NF.GT.0) THEN
            DO 160 I = 1,MF
               W(I) = W(IRNF(IF1+I-1))
  160       CONTINUE
            CALL MA50H(TRANS,MF,NF,FACT(IF1),MF,IRNF(IF1+MF),W,ICNTL(5))
            DO 170 I = 1,NF
               X(NP+I) = W(I)
  170       CONTINUE
         ELSE
...
C         Permute X
  ...
  240       CONTINUE
         END IF
         IF (MP.GT.0 .AND. ICNTL(3).GT.3) WRITE (MP,
     +       '(A/(4X,5F10.4))') ' Leaving MA50C with X =', (X(I),I=1,N)
      END IF
      RETURN
C Error condition.
  250 INFO(1) = -1
      IF (LP.GT.0) WRITE (LP,'(/A/2(A,I8))')
     +    ' **** Error return from MA50C ****',' M =',M,' N =',N
      END

      SUBROUTINE MA50D(LA,A,IND,IPTR,N,DISP,REALS)
...
C First entry of row(column) has been located.
   ...
   20 CONTINUE
      ...
      SUBROUTINE MA50E(M,N,A,LDA,PIVTOL,IPIV,RANK)
**
...
*  =======
*
*  MA50E computes an LU factorization of a general m-by-n matrix A.
...
*        Update elements in column J.
         DO 10 I = 1,J - 1
            IF (M.GT.I) CALL SAXPY(M-I,-A(I,J),A(I+1,I),1,A(I+1,J),1)
   10    CONTINUE
...
*           Apply row interchange to columns 1:N+J-K.
            IF (JP.NE.J) CALL SSWAP(N+J-K,A(J,1),LDA,A(JP,1),LDA)
*
...
*           Apply column interchange and record it.
            IF (K.LT.N) CALL SSWAP(M,A(1,J),1,A(1,N-K+J),1)
            IPIV(N-K+J) = -J
*
         END IF
*
...
*
      ...
      SUBROUTINE MA50F(M,N,A,LDA,PIVTOL,IPIV,RANK)
*
...
*  =======
*
*  MA50F computes an LU factorization of a general m-by-n matrix A.
...
*           Update diagonal and subdiagonal elements in column J.
            CALL SGEMV('No transpose',M-J+1,J-1,-ONE,A(J,1),LDA,A(1,J),
     +                 1,ONE,A(J,J),1)
*          Find pivot.
            JP = J - 1 + ISAMAX(M-J+1,A(J,J),1)
            IPIV(J) = JP
...
*             Compute block row of U.
               CALL SGEMV('Transpose',J-1,N-J,-ONE,A(1,J+1),LDA,A(J,1),
     +                    LDA,ONE,A(J,J+1),LDA)
            END IF
...
*
      ...
      SUBROUTINE MA50G(M,N,A,LDA,NB,PIVTOL,IPIV,RANK)
*
...
*  =======
*
*  MA50G computes an LU factorization of a general m-by-n matrix A.
*
*  The factorization has the form
*     A = P * L * U * Q
*  where P is a permutation matrix of order m, L is lower triangular
...
* TEMP Temporary variable for swaps.
      ...
      EXTERNAL SGEMM,SGEMV,SSWAP,SSCAL,STRSM,STRSV
...
*          Update diagonal and subdiagonal elements in column J.
            CALL SGEMV('No transpose',M-J+1,J-J1,-ONE,A(J,J1),LDA,
...
*           Apply row interchange to columns J1:J2
            IF (JP.NE.J) CALL SSWAP(J2-J1+1,A(J,J1),LDA,A(JP,J1),LDA)
*
*           Compute elements J+1:M of J-th column.
            IF (J.LT.M) CALL SSCAL(M-J,ONE/A(J,J),A(J+1,J),1)
...
*             Compute row of U within current block
               CALL SGEMV('Transpose',J-J1,J2-J,-ONE,A(J1,J+1),LDA,
...
*           Apply column interchange.
            IF (K.NE.N) CALL SSWAP(M,A(1,J),1,A(1,N-K+J),1)
            IF (N-K+J.GT.J2) THEN
*              Apply operations to new column.
               DO 20 I = J1,J - 1
                  ...
                  A(JP,J) = TEMP
   20          CONTINUE
               IF(J.GT.J1) CALL STRSV('Lower','No transpose','Unit',
...
*              Update the Schur complement
               CALL STRSM('Left','Lower','No transpose','Unit',J2-J1+1,
     +                    N-K,ONE,A(J1,J1),LDA,A(J1,J2+1),LDA)
               IF (M.GT.J2) CALL SGEMM('No transpose','No transpose',
...
*
      ...
      SUBROUTINE MA50H(TRANS,M,N,A,LDA,IPIV,B,ICNTL5)
*
...
*        Solve L*X = B, overwriting B with X.
         IF (ICNTL5.GT.0) THEN
            IF (RANK.GT.0) CALL STRSV('L','NoTrans','Unit',RANK,A,LDA,B,
     +                                1)
         ELSE
            DO 40 K = 1,RANK - 1
               IF (B(K).NE.ZERO) CALL SAXPY(RANK-K,-B(K),A(K+1,K),1,
     +                                B(K+1),1)
   40       CONTINUE
         END IF
...
*        Solve U*X = B, overwriting B with X.
         IF (ICNTL5.GT.0) THEN
            IF (RANK.GT.0) CALL STRSV('U','NoTrans','NonUnit',RANK,A,
     +                                LDA,B,1)
         ELSE
            DO 50 K = RANK,2,-1
               IF (B(K).NE.ZERO) THEN
...
*
         IF (ICNTL5.GT.0) THEN
            IF (RANK.GT.0) CALL STRSV('U','Trans','NonUnit',RANK,A,LDA,
     +                                B,1)
         ELSE
            IF (RANK.GT.0) B(1) = B(1)/A(1,1)
            DO 90 I = 2,RANK
               TEMP = B(I) - SDOT(I-1,A(1,I),1,B(1),1)
               B(I) = TEMP/A(I,I)
   90       CONTINUE
         END IF
...
*        Solve L'*X = B, overwriting B with X.
         IF (ICNTL5.GT.0) THEN
            IF (RANK.GT.0) CALL STRSV('L','Trans','Unit',RANK,A,LDA,B,1)
         ELSE
            DO 100 I = RANK - 1,1,-1
               B(I) = B(I) - SDOT(RANK-I,A(I+1,I),1,B(I+1),1)
  100       CONTINUE
         END IF
...
!
      SUBROUTINE MA48A(M,N,NE,JOB,LA,A,IRN,JCN,KEEP,CNTL,ICNTL,IW,INFO,
     +                 RINFO)
...
C       LASTC is also used to get workspace in KEEP for MA50A.
...
C       LENC is also used to get workspace in KEEP for MA50A.
...
C NDIAG number entries placed on diagonal by MC21A.
...
C
C .. Externals ..
      EXTERNAL MC13D,MC21A,MA50A
C MC13D    finds permutation for block triangular form
C MC21A    finds permutation for zero-free diagonal
...
C Check column permutation
 ...
 9000       FORMAT (' Entries:'/3 (1P,E12.4,2I6))
         ELSE
...
C Get workspace from KEEP for MC21A and MC13D
      IW21 = 2*N
      IW13 = IW21
C Partition IW for arrays used by MC21A and MC13D
      IPTRA = LASTC + N
      LENC = IPTRA + N
      IB = LENC
      IP = LENC + N
...
C   Compute permutation for zero-free diagonal
      CALL MC21A(N,IRN(NE+1),NZA,IW(IPTRA+1),IW(LENC+1),IW(IP+1),NDIAG,
     +           KEEP(IW21+1))
...
C KEEP(M+1) .. col permutation ...new.to.old
      CALL MC13D(N,IRN(NE+1),NZA,IW(IPTRP+1),IW(LENP+1),KEEP(M+1),
...
C Repartition IW (none of previous data is needed any more)
      ...
      NEXTC = 6*M + 2*N
      PTRO = NEXTC
C Use scratch space in KEEP for work arrays in MA50A.
      LASTC = M + N
      LENC = LASTC + N
C
...
C Control for forcing columns to end.
            ...
            CALL MA50A (NR,NC,NZB,LA-NE-K,A(NE+K+1),IRN(NE+K+1),
...
C KEEP(LENC+KB) holds the actual storage forecast for MA50B entry.
            KB = KB + 1
...
C Set rank estimate to min of structural and INFO(5) estimate.
 ...
 9160 FORMAT (' Message from MA48A/AD ..',I8,' out-of-range indices fo',
     +       'und')
      ...
      SUBROUTINE MA48B (M,N,NE,JOB,LA,A,IRN,JCN,KEEP,CNTL,ICNTL,W,IW,
     +                  INFO,RINFO)
...
C Shift the indices and pointers to local values.
  ...
  120    CONTINUE
         NP = KEEP(MBLOCK+3*JB)
         CALL MA50B (NR,NC,NZB,JOB5,A(K+1),IRN(K+1),KEEP(IPTRD+J1),
...
C Full blocks
 ...
 9190 FORMAT (' Error return from MA48B/BD because ','JOB = ',I10)
      ...
      SUBROUTINE MA48C (M,N,TRANS,JOB,LA,A,IRN,KEEP,CNTL,ICNTL,RHS,X,
     +                  ERROR,W,IW,INFO)
...
C     .. Local variables ..
     ...
     +        J3,K,KASE,KB,KBLOCK,KK,KEEP71(5)
      ...
      REAL OLDOMG(2),OMEGA(2),OM1,OM2,TAU
...
C Full blocks
 ...
 9110          FORMAT (' RHS =  ',1P,5E12.4/ (8X,5E12.4))
            ELSE
...
C Solve system using MA48D/DD or MA50C/CD.
         IF (NB.EQ.1 .AND. KEEP(MBLOCK+3).GE.0) THEN
            CALL MA50C (M,N,ICNTL5,IQB,KEEP(MBLOCK+3),TRANS,LA-NE,
...
C We are calculating the infinity norm of INV(A).W
               CALL MC71A(N,KASE,W(3*NEQ+1),COND(K),RHS,IW,KEEP71)
C
...
C Solve system using MA48D/DD or MA50C/CD.
                  IF (NB.EQ.1 .AND. KEEP(MBLOCK+3).GE.0) THEN
                     CALL MA50C(M,N,ICNTL5,IQB,KEEP(MBLOCK+3),
     +                           .NOT.TRANS,LA-NE,A(NE+1),IRN(NE+1),
     +                           KEEP(IPTRL+1),KEEP(IPTRU+1),W(3*NEQ+1),
     +                           W(2*NEQ+1),RHS,INFO)
                  ELSE
                     CALL MA48D(N,NE,LA-NE,A(NE+1),A,IRN(NE+1),IRN,
...
C Solve system using MA48D/DD or MA50C/CD.
                  IF (NB.EQ.1 .AND. KEEP(MBLOCK+3).GE.0) THEN
                     CALL MA50C(M,N,ICNTL5,IQB,KEEP(MBLOCK+3),TRANS,
...
C Permute solution vector
 ...
 9120       FORMAT (' ERROR =',1P,3E12.4)
         END IF
         IF (ICNTL(3).GT.3) THEN
            WRITE (MP,9130) (X(I),I=1,NVAR)
 9130       FORMAT (' X =    ',1P,5E12.4:/ (8X,5E12.4))
         ELSE
 ...
 9170 FORMAT (' Error return from MA48C/CD because of ','nonconvergenc',
     ...
     +       'h ICNTL','(9) = ',I10)
      ...
      SUBROUTINE MA48D(N,NE,LA,A,AA,IRN,IRNA,IPTRD,IPTRO,NB,IBLOCK,
...
C I     row index
C IFLAG error flag from MA50C .. will never be set
C IQB   is passed to MA50C/CD to indicate that no column permutation
C       is used.
C J     column index
C JB    block index.
...
CDIR$ IVDEP
   ...
   20          CONTINUE
            ELSE
               ...
               CALL MA50C(NC,NC,ICNTL5,IQB,IBLOCK(2,JB),TRANS,LA+1-J1,
