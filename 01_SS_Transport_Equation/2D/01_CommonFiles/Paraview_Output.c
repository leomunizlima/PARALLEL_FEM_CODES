...
int Paraview_Output(int RANK, int NPROC, ParametersType *Parameters, FemStructsType *FemStructs, FemFunctionsType *FemFunctions)
{
	...
	sprintf(FileName, "%s/%02d/RANK_%02d_%s_%s_%s_%s_%s_%s_N%d_E%d.vts",Parameters->arguments[3],NPROC,RANK,Parameters->ProblemTitle,
		Parameters->StabilizationForm,Parameters->ShockCapture, Parameters->h_Shock,Parameters->MatrixVectorProductScheme,
...
//			fprintf(OutFile,"\t\t\t\t   %.12f\t%.12f\t%.1f\n", Node[I].x,Node[I].y, 0.0);
	...
	if (RANK == 0){
		FileName[0] ='\0';
		sprintf(FileName, "%s/%02d/%s_%s_%s_%s_%s_%s_N%d_E%d.pvd",Parameters->arguments[3],NPROC,Parameters->ProblemTitle,
		Parameters->StabilizationForm,Parameters->ShockCapture, Parameters->h_Shock,Parameters->MatrixVectorProductScheme,
		Parameters->Preconditioner,Parameters->nNodes,Parameters->nEl); 	
		OutFile = myfopen(FileName,"w");
		...
		for (I=0;I<NPROC;I++){	
			FileName[0] ='\0';
			sprintf(FileName, "../03_output/%02d/RANK_%02d_%s_%s_%s_%s_%s_%s_N%d_E%d.vts",NPROC,I,Parameters->ProblemTitle,
				Parameters->StabilizationForm,Parameters->ShockCapture,	Parameters->h_Shock, 
